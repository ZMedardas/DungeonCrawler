package
{
	...
	public class troll extends MovieClip
	{
		var hp:int=new int;
		...
		var sprite:Bitmap=new Bitmap(new BitmapData(50,150,true,0x00000000));
		var sprite_h:Sprite=new Sprite;
		var sp_rect:Rectangle=new Rectangle(0,0,50,150);
		var sp_p:Point=new Point(0,0);
		var time:int=0;
		var at_time:int=0;
		var frame:int=0;
		
		public function troll(X:int,Y:int,level_num:int):void
		{
			posX=X;
			posY=Y;
			...
			sprite.x-=25;
			sprite.y-=75;
			sprite.bitmapData.copyPixels(new troll_sp,sp_rect,sp_p,null,null,true);
		}
			...
			if(stuned<=0)
			{
				if(timer==0)
				{
					...
					else
					{
						...
						{
							sp_rect.x=0;
							sprite.bitmapData=new BitmapData(50,150,true,0x00000000);
							sprite.bitmapData.copyPixels(new troll_sp,sp_rect,sp_p,null,null,true);
							at_time--;
						}
