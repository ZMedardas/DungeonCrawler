package
{
	...
	public class bomb extends MovieClip
	{
		...
		public function bomb(X:int,Y:int,level_num:int):void
		{
			posX=X;
			posY=Y;
			hp_total=hp=10*level_num;
			addChild(sprite_h);
			...
			sprite.x-=25;
			sprite.y-=25;
			sprite.bitmapData.copyPixels(new dog_sp,sp_rect,sp_p,null,null,true);
		}
		...
		public function actions():void
		{
			...
			if(time>5)
			{
				...
				sprite.bitmapData.copyPixels(new bom_sp,sp_rect,sp_p,null,null,true);
			}
			if(stuned<=0)
			{
				if(timer==0)
				{
					...
					{
						if(Math.abs(345-this.x)<10&&Math.abs(205-this.y)<10)blow_up();
						if(wait_time>10)
						{
		...
		public function dir():void
		{
			...
			if(path.length==0)doc.enemies[posX][posY]=ar;
		}
		...
		public function damage_recieve(dmg:int,physical:Boolean,st:int):void
		{
			...
			{
				hp_bar.width=40*(hp/hp_total);
				stuned+=st;
			}
		}
		public function blow_up()
		{
			if(Math.abs(345-this.x)<10&&Math.abs(205-this.y)<10)
			{
				var even:AttackEvent=new AttackEvent("DAMAGE_RECIEVE",att,false);
				dispatchEvent(even);
				var even1:KillEvent=new KillEvent("ENEMY_DEAD",Math.floor(0));
				dispatchEvent(even1);
			}
			else
			{
				var even1:KillEvent=new KillEvent("ENEMY_DEAD",Math.floor((hp_total+att+def+mag_def)/10));
				dispatchEvent(even1);
			}
			removeChild(sprite_h);
			doc.enemies[posX][posY]=0;
			doc.enemies_cur.splice(doc.enemies_cur.indexOf(this,0),1);
			doc.particles.push(new bom_spl);
			addChild(doc.particles[doc.particles.length-1]);
			doc.particles[doc.particles.length-1].addEventListener(Event.REMOVED,del_init);
		}
		public function del_init(e:Event)
		{
			del();
		}
